...
import { SUBJECTS, type Question, type Subject, type SubjectKey } from "@/lib/quizData";
import {
  getBadge,
  loadProgress,
  resetProgress,
...
function CompleteScreen({ subjectKey }: { subjectKey: SubjectKey }) {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-12 animate-fade-up">
      <div className="grid gap-6">
        ...
        {APP_CONFIG.showExamFeedback && <ExamFeedback subject={subject} />}
      </div>
    </div>
  );
}

function ExamFeedback({ subject }: { subject: Subject }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-card/90 p-6 shadow-elevated backdrop-blur sm:p-8">
      <div className={`absolute -right-16 -top-16 h-40 w-40 rounded-full ${subject.themeBg} opacity-25 blur-2xl`} />
      <div className={`absolute -bottom-12 -left-10 h-32 w-32 rounded-full ${subject.themeBg} opacity-20 blur-2xl`} />
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div>
          ...
          <h3 className="text-2xl font-extrabold tracking-tight">Tell us how it went</h3>
          <p className="mt-2 text-sm text-muted-foreground">
            Share your name and a short message. Your response stays right here on this page.
          </p>
          ...
          <form onSubmit={submitFeedback} className="mt-5 space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                className={`h-11 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none transition focus-visible:ring-2 ${subject.themeRing}`}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={4}
                className={`w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none transition focus-visible:ring-2 ${subject.themeRing}`}
              />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className={`rounded-xl px-5 py-2.5 text-sm font-bold shadow-soft transition disabled:cursor-not-allowed disabled:opacity-50 ${subject.themeBtn}`}
              >
        ...
        <div className="rounded-2xl border border-border bg-gradient-to-br from-white/80 via-white/60 to-white/30 p-5 shadow-soft sm:p-6">
          ...
          <p className="mt-3 text-sm leading-relaxed text-foreground/90">
            May this site be helpful for your exam preparation, and thank you for using it. If you
            need any other kind of stuff, let me know, and thanks for your support. Wish you all the
            best.
          </p>
          <div className={`mt-4 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${subject.themeSoft} ${subject.themeText}`}>
            <Sparkles className="h-3.5 w-3.5" />
            Premium study vibes
          </div>
        </div>
